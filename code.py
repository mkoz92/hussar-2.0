...
if __name__ == "__main__":
    ...
    NUM_PIXELS = 5
    strip1 = NeoPixelBackground(board.NEOPIXEL0, NUM_PIXELS)
    strip2 = NeoPixelBackground(board.NEOPIXEL1, NUM_PIXELS)
    strip3 = NeoPixelBackground(board.NEOPIXEL2, NUM_PIXELS)

    def fill_all_strips(num, value):
        strip1[num] = value
        strip2[num] = value
        strip3[num] = value
    ...
    def light_up_all():
        for brightness in range(255):
            val = (brightness, brightness, brightness)
            for i in range(len(strip1)):
                fill_all_strips(i, val)
                time.sleep(0.001)
            time.sleep(0.002)
    ...
    def pulse_all():
        for brightness in range(255):
            ...
            for i in range(len(strip1)):
                ...
                time.sleep(0.0009)
            time.sleep(0.001)
        for brightness in reversed(range(255)):
            val = (brightness, brightness, brightness)
            for i in range(len(strip1)):
                fill_all_strips(i, val)
                time.sleep(0.0009)
    ...
    while True:
        strip1[2:4] = [(0, 0, 200), (0, 0, 200)]
        strip2[2:4] = [(220, 0, 80), (220, 0, 80)]
        strip3[2:4] = [(80, 220, 10), (80, 220, 10)]

        time.sleep(1)
        strip1[1:2] = [(0, 0, 200)]
        strip2[2:3] = [(220, 0, 80)]
        strip3[3:4] = [(80, 220, 10)]
